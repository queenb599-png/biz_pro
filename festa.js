...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const saved = localStorage.getItem('festgo_state_light');
    if (saved) {
        try {
            const parsed = JSON.parse(saved);
            appState.points = parsed.points ?? 1500;
            appState.level = parsed.level ?? 1;
            appState.xp = parsed.xp ?? 0;
            appState.quests.forEach((q, idx) => {
                if (parsed.quests && parsed.quests[idx]) {
                    q.completed = parsed.quests[idx].completed;
                    q.completedDate = parsed.quests[idx].completedDate;
                }
            });
            appState.bookedTickets = parsed.bookedTickets ?? [];
            appState.coupons = parsed.coupons ?? [];
        } catch (e) {
...
function saveState() {
    localStorage.setItem('festgo_state_light', JSON.stringify({
        points: appState.points,
        level: appState.level,
        xp: appState.xp,
        quests: appState.quests.map(q => ({ completed: q.completed, completedDate: q.completedDate })),
        bookedTickets: appState.bookedTickets,
